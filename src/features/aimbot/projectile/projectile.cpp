...
CBaseEntity *CAimbotProjectile::GetCurrentTarget () { return m_pTarget; }

std::vector<Vector> &CAimbotProjectile::GetPath () { return m_vecPath; }

float CAimbotProjectile::GetInitialZOffset (CTFWeaponBase *pWeapon,
...
bool CAimbotProjectile::SolveBallisticArc (Vector &outAngle, const Vector p0,
...
bool CAimbotProjectile::CheckTrajectory (
    CBaseEntity *pTarget, const Vector vecStartPos, const Vector vecTargetPos,
    const Vector vecAngle, const ProjectileInfo_t &prjInfo, float flGravity)
{
	...
	while (flClock < flMaxTime)
	{
		...
		helper::engine::TraceHull (prevPos, vecPos, vecMins, vecMaxs,
...
bool CAimbotProjectile::GetProjectileInfo (ProjectileInfo_t &pOut,
					   CTFPlayer *pLocal,
					   CTFWeaponBase *pWeapon)
{
	...
	float flGravity
	    = interfaces::Cvar->FindVar ("sv_gravity")->GetFloat () / 800;
	...
	case TF_WEAPON_ROCKETLAUNCHER_DIRECTHIT:
	{
		pOut.hull.Set ();
		pOut.speed
		    = pLocal->InCond (TF_COND_RUNE_PRECISION)
			  ? 3000
			  : AttributeHookValue (1100, "mult_projectile_speed",
						pWeapon, nullptr, true);
		pOut.offset.x = 23.5f;
		pOut.offset.y = AttributeHookValue (0, "centerfire_projectile",
						    pWeapon, nullptr, true)
					== 1
				    ? 0
				    : 12;
	...
	case TF_WEAPON_DRG_POMSON:
	{
		...
		pOut.hull
		    = bIsCowMangler ? Vector (0, 0, 0) : Vector (1, 1, 1);
		pOut.simple_trace = true;
		return true;
	}
	...
	case TF_WEAPON_CANNON:
	{
		...
		float mortar   = bIsCannon
				     ? AttributeHookValue (
					   0.f, "grenade_launcher_mortar_mode",
					   pWeapon, nullptr, true)
				     : 0;
		pOut.speed     = AttributeHookValue (
		    pLocal->InCond (TF_COND_RUNE_PRECISION)
			? 3000
			: AttributeHookValue (1200, "mult_projectile_range",
					      pWeapon, nullptr, true),
	...
	case TF_WEAPON_PIPEBOMBLAUNCHER:
	{
		...
		float m_flChargeBeginTime
		    = ((CTFPipebombLauncher *)pWeapon)->m_flChargeBeginTime ();
		...
		pOut.speed = AttributeHookValue (
		    Math::RemapVal (
			0, 0,
			AttributeHookValue (4.0f, "stickybomb_charge_rate",
					    pWeapon, nullptr, true),
			900, 2400, true),
		    "mult_projectile_range", pWeapon, nullptr, true);
		return true;
	...
	case TF_WEAPON_FLAREGUN:
	{
		...
		pOut.hull.Set (0, 0, 0);
		pOut.speed = AttributeHookValue (2000, "mult_projectile_speed",
						 pWeapon, nullptr, true);
		pOut.gravity  = 0.01f;
	...
	case TF_WEAPON_COMPOUND_BOW:
	{
		...
		float flchargebegintime
		    = static_cast<CTFPipebombLauncher *> (pWeapon)
			  ->m_flChargeBeginTime ();
		float charge = 0.0f;
		if (flchargebegintime > 0)
			charge = TICKS_TO_TIME (pLocal->GetTickBase ())
				 - flchargebegintime;

		pOut.speed = Math::RemapVal (charge, 0, 1, 1800, 2600);
		pOut.gravity
		    = Math::RemapVal (charge, 0, 1, 0.5, 0.1) * flGravity;
		pOut.lifetime = 10;
		return true;
	}
	...
	{
		static ConVar *tf_flamethrower_size
		    = interfaces::Cvar->FindVar ("tf_flamethrower_size");
	...
	{
		static ConVar *tf_scout_stunball_base_speed
		    = interfaces::Cvar->FindVar (
...
std::vector<PotentialTarget>
CAimbotProjectile::GetBestTargets (CTFPlayer *pLocal, CTFWeaponBase *pWeapon)
{
	auto vTargets = AimbotUtils::GetTargets (pWeapon->CanHitTeammates (),
						 pLocal->m_iTeamNum ());
	if (vTargets.empty ())
		return {};
	...
	for (const auto &targetEntry : vTargets)
	{
		...
		vPotentialTargets.emplace_back (PotentialTarget{
		    vecDir, vecCenter, flDistance, flFov, targetEntry.ptr });
	}
	...
	std::sort (vPotentialTargets.begin (), vPotentialTargets.end (),
		   [&] (PotentialTarget a, PotentialTarget b) {
			   return a.fov < b.fov;
		   });
...
void CAimbotProjectile::RunMain (CTFPlayer *pLocal, CTFWeaponBase *pWeapon)
{
	...
	bool bVisualsEnabled
	    = Settings::Aimbot.path || Settings::Aimbot.indicator;
	...
	if (sv_gravity == nullptr)
		return Logs::Error (
		    "[CAimbotProjectile::RunMain] sv_gravity is null!");
	...
	float flPrimeTime
	    = pWeapon->GetWeaponID () == TF_WEAPON_PIPEBOMBLAUNCHER ? 0.7f : 0;
	...
	for (const auto &target : vTargets)
	{
		...
		{
			CTFPlayer *pPlayer
			    = static_cast<CTFPlayer *> (target.entity);
			...
			vecAimPos.z += GetInitialZOffset (
			    pWeapon, pPlayer->m_vecMaxs ());
		} else
		{
		...
		if (prjInfo.simple_trace)
		{
			...
			Math::AngleVectors (vecAngle, &vecForward, &vecRight,
					    &vecUp);
			...
			if (!CheckTrajectory (target.entity, vecSpawnPos,
					      vecAimPos, vecAngle, prjInfo,
					      0.0f))
				continue;
		...
		{
			float flGravity
			    = sv_gravity->GetFloat () * 0.5f * prjInfo.gravity;

			Vector vecAngle;
			if (!SolveBallisticArc (vecAngle, vecEyePos, vecAimPos,
						prjInfo.speed, flGravity))
				continue;
			...
			m_vecAimPos
			    = vecAimPos
			      + Vector{ 0, 0, GetAimDrop (flGravity, flTime) };
			m_vecPath		    = vPath;
			EntityList::m_pAimbotTarget = target.entity;
			return;
		}
...
void CAimbotProjectile::RunAim (CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
				CUserCmd *pCmd, AimbotState &pState)
{
	...
	if (helper::localplayer::CanShoot (pLocal, pWeapon, pCmd))
	{
		if ((pCmd->buttons & IN_ATTACK))
		{
			...
			{
				float flchargebegintime
				    = static_cast<CTFPipebombLauncher *> (
					  pWeapon)
					  ->m_flChargeBeginTime ();
	...
	if (helper::localplayer::IsAttacking (pLocal, pWeapon, pCmd))
	{
		...
		AimbotMode mode
		    = static_cast<AimbotMode> (Settings::Aimbot.mode);
...
void CAimbotProjectile::RunIndicator ()
{
	...
	if (Settings::Aimbot.indicator
	    == static_cast<int> (AimbotIndicatorStyle::NONE))
		return ResetIndicator ();
	...
	CTFWeaponBase *pWeapon
	    = HandleAs<CTFWeaponBase *> (pLocal->GetActiveWeapon ());
	if (pWeapon == nullptr)
		return ResetIndicator ();
	...
	m_vecOldIndicatorPos = m_vecOldIndicatorPos.Lerp (
	    m_vecAimPos, interfaces::GlobalVars->frametime * 10.0f);
	...
	if (helper::engine::WorldToScreen (m_vecOldIndicatorPos, screenPos))
	{
		...
		switch (static_cast<AimbotIndicatorStyle> (
		    Settings::Aimbot.indicator))
		{
		...
		case AimbotIndicatorStyle::CIRCLE:
		{
			interfaces::Surface->DrawOutlinedCircle (
			    (int)screenPos.x, (int)screenPos.y, iSize, 68);
			break;
		}
...
{
	if (!Settings::Aimbot.path || m_pTarget == nullptr
	    || m_vecPath.empty ())
		return;
...
void CAimbotProjectile::DrawPath (const std::vector<Vector> &vPath)
{
	for (int i = 1; i < vPath.size (); i++)
	{
		...
		bool bIsPreviousVisible = helper::engine::WorldToScreen (
		    vPath[i - 1], vecPrevScreen);
		bool bIsCurrentVisible
		    = helper::engine::WorldToScreen (vPath[i], vecCurrScreen);

		if (bIsPreviousVisible && bIsCurrentVisible)
			interfaces::Surface->DrawLine (
			    vecPrevScreen.x, vecPrevScreen.y, vecCurrScreen.x,
			    vecCurrScreen.y);
	}
}
