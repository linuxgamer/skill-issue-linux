...
void Vector2_RegisterClass(asIScriptEngine* engine)
{
	...
	engine->RegisterObjectBehaviour(VEC2_CLASSNAME, asBEHAVE_CONSTRUCT, "void f()", asFUNCTION(Vector2_Default), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectBehaviour(VEC2_CLASSNAME, asBEHAVE_CONSTRUCT, "void f(float, float)", asFUNCTION(Vector2_Init), asCALL_CDECL_OBJFIRST);
