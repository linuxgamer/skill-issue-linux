...
std::string GetTextureGroupName(IMaterial* mat)
{
	if (!mat)
	{
		Logs::Error("AS: Calling on a invalid material! (mat->GetTextureGroupName)");
		return INVALID_MAT;
	}
...
{
	if (!mat) return Logs::Error("AS: Calling on a invalid material! (mat->SetMaterialVarFlag)");
...
bool GetMatVarFlag(IMaterial* mat, int flag)
{
	if (!mat)
	{
		Logs::Error("AS: Calling on a invalid material! (mat->GetMaterialVarFlag)");
		return false;
	}
...
{
	if (!mat) return Logs::Error("AS: Calling on a invalid material! (mat->SetAlphaModulation)");
...
float GetAlphaMod(IMaterial* mat)
{
	if (!mat)
	{
		Logs::Error("AS: Calling on a invalid material! (mat->GetAlphaModulation)");
		return 0.0f;
	}
