...
void GameEvent_RegisterClass(asIScriptEngine* engine)
{
	...
	engine->RegisterObjectBehaviour(GAMEEVENT_CLASSNAME, asBEHAVE_ADDREF, "void f()", asFUNCTION(GameEvent_AddRef), asCALL_CDECL_OBJLAST);
	engine->RegisterObjectBehaviour(GAMEEVENT_CLASSNAME, asBEHAVE_RELEASE, "void f()", asFUNCTION(GameEvent_Release), asCALL_CDECL_OBJLAST);
