...
bool Add (const std::string &event, const std::string &id,
...
void ExecuteHook (const ASHook &hook,
		  const std::function<void (asIScriptContext *)> &argSetter)
{
...
void Hooks_RegisterLibrary (asIScriptEngine *engine)
{
	...
	engine->RegisterFuncdef (
	    "void CalcViewModelHook(Vector3 position, Vector3 angle)");
	engine->RegisterFuncdef ("void GenericHook()");
	engine->RegisterFuncdef (
	    "void LevelInitPreEntHook(const string &in mapName)");
	...
	engine->SetDefaultNamespace ("Hooks");
	{
		...
		engine->SetDefaultAccessMask (
		    ScriptAccessMask::SCRIPT_MASK_ALLOW_HOOKS);
		engine->RegisterGlobalFunction (
		    "bool Add(const string &in event, const string &in id, "
		    "CreateMoveHook @cb)",
		...
		engine->RegisterGlobalFunction (
		    ...
		    asFUNCTION (Add), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "bool Remove(const string &in event, const string &in id)",
