...
void Materials_RegisterLibrary(asIScriptEngine *engine)
{
	...
	engine->SetDefaultNamespace("Materials");
	{
		...
		engine->RegisterGlobalFunction("Texture@ CreateTextureNamedRenderTarget(const string &in name, int width, int height)", asFUNCTION(CreateTextureNamedRenderTarget), asCALL_CDECL);
	}
