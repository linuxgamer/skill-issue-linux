...
{
	return ImGui::BeginChild(str_id.c_str(), ImVec2(size.x, size.y), (ImGuiChildFlags)child_flags, (ImGuiWindowFlags)window_flags);
}

bool BeginChildB(unsigned int id, const Vec2& size = Vec2(0, 0), int child_flags = 0, int window_flags = 0)
{
	return ImGui::BeginChild((ImGuiID)id, ImVec2(size.x, size.y), (ImGuiChildFlags)child_flags, (ImGuiWindowFlags)window_flags);
}
...
bool Combo(const std::string& label, int& current_item, CScriptArray* items, int popup_max_height_in_items)
{
	...
	return ImGui::Combo(label.c_str(), &current_item, c_items.data(), static_cast<int>(c_items.size()), popup_max_height_in_items);
}
