...
static void DrawAttachments(CBaseEntity* pEntity)
{
	Color color{};
	int passes = 0;
	auto moveChild = pEntity->FirstShadowChild();

	float flColor[3] = {1, 1, 1};

	while (moveChild != nullptr && passes <= MAX_PASSES)
	{
		...
		Chams::m_Entities.insert(attachment->GetIndex());
