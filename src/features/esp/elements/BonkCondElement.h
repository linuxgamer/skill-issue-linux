...
class BonkedCondElement : public IBaseElement
{
public:
	bool ShouldDraw(CBaseEntity* pEnt, const ESP_Data&) const override
	{
		...
		return pPlayer->InCond(ETFCond::TF_COND_BONKED) && (Settings::ESP.fconditions & static_cast<int>(ESPConditionFlags::Bonked));
	}

	void Draw(Vec2& pos, CBaseEntity* ent, const ESP_Data& data, ESPContext& ctx) const override
	{
		Color color = GetColor(ent, data);
		helper::draw::TextShadow(pos.x, pos.y, color, "Bonk");
	}

	Vec2 GetSize(const ESP_Data& data) const override
	{
		int w, h;
		helper::draw::GetTextSize("Bonk", w, h);
		return Vec2(w, h);
	}
