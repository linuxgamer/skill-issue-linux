...
{
	void DrawContents ();
	void DrawHealthbar (ImDrawList *draw, ImVec2 pos, int health,
			    int maxhealth, int iconSize);
	...
	float GetRadius ();
	int GetRange ();
	Vec2 WorldToRadar (const Vector &localPos, const Vector &enemyPos,
			   float viewAnglesYaw);
};
...
Vec2 Radar::WorldToRadar (const Vector &localPos, const Vector &enemyPos,
			  float viewAnglesYaw)
{
...
float Radar::GetRadius () { return m_flRadius; }

int Radar::GetRange () { return m_iRange; }
...
void Radar::DrawContents ()
{
	...
	draw->AddLine ({ center.x, pos.y }, { center.x, pos.y + size },
		       IM_COL32 (255, 255, 255, 40));
	draw->AddLine ({ pos.x, center.y }, { pos.x + size, center.y },
		       IM_COL32 (255, 255, 255, 40));
	...
	for (const auto &entry : EntityList::GetEntities ())
	{
		...
		if (entry.flags & EntityFlags::IsBuilding
		    && !Settings::Radar.buildings)
			continue;

		if (entry.flags & EntityFlags::IsPlayer
		    && (!Settings::Radar.players
			|| !static_cast<CTFPlayer *> (entry.ptr)->IsAlive ()))
			continue;

		if (entry.flags & EntityFlags::IsProjectile
		    && !Settings::Radar.projectiles)
			continue;

		Vec2 p = WorldToRadar (localPos, entry.ptr->GetAbsOrigin (),
				       viewYaw);
		Color color = ESP_Utils::GetEntityColor (entry.ptr);

		draw->AddCircleFilled (
		    { center.x + p.x, center.y - p.y }, iconSize,
		    IM_COL32 (color.r (), color.g (), color.b (), color.a ()));
	}
}

void Radar::DrawHealthbar (ImDrawList *draw, ImVec2 pos, int health,
			   int maxhealth, int iconSize)
{
	constexpr int barOffset = 3;
	draw->AddRectFilled (
	    ImVec2 (pos.x - iconSize, pos.y + barOffset),
